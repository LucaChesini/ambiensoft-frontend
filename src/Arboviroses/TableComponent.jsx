...
const TableComponent = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [arboviroses, bairros, ruas] = await Promise.all([
          axios.get('http://127.0.0.1:8000/api/enums/arboviroses'),
          axios.get('http://127.0.0.1:8000/api/enderecos/bairro'),
          axios.get('http://127.0.0.1:8000/api/enderecos/rua'),
        ]);
        setDataFiltro({
          arboviroses: Object.entries(arboviroses.data),
          ruas: ruas.data.data,
  ...
  const fetchData = async (params = {}) => {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/arboviroses/arbovirose', { params });
  ...
  const deletar = async (id, tipo) => {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/api/arboviroses/${tipo}/${id}`);
      fetchData();
    } catch (err) {
      setError('Erro ao buscar dados');
    } finally {
        ...
        <div className="basis-1/2 mr-4">
          <FormControl sx={{ width: '100%' }}>
            ...
            >
              ...
              {dataFiltro.arboviroses.map(([key, value]) => (
        ...
        <Table stickyHeader>
          ...
          <TableBody>
            {data.map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  <Link to={`/arboviroses/${row.doenca}/${row.id}`} >
                    {row.nome}
                  </Link>
                </TableCell>
                ...
                <TableCell>
                  ...
                  variant="contained" 
                  color='info'>
                    <Link to={`/arboviroses/${row.doenca}/${row.id}/editar`}>
                      <EditIcon />
                    </Link>
                  </Button>
                </TableCell>
