...
const EditarDengue = () => {
    ...
    const mapApiResponseToFormData = (response) => {
        ...
        return {
            ...
            rua_id: data.rua_id || "",
            numero: data.numero || "",
            sinais: data.arbovirosable?.dengue_sinals?.map((sinal) => sinal.id) || [],
            doencas: data.arbovirosable?.dengue_doencas?.map((doenca) => doenca.id) || [],
        };
    };
    ...
    const handleSubmit = async (event) => {
        ...
        handleDateChange();
        try {
            const response = await axios.put(`http://127.0.0.1:8000/api/arboviroses/dengue/${id}`, formData);
            setFormErrors({});
